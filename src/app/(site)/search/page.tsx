 ...
 *
 * ─────────────────────────────────────────────────────────────
 * **이 파일은 self-fetch 원칙(`src/lib/api/serverFetch.ts`)의 유일한 예외다.**
 * 검색 결과만 `fetchApi("/api/pages/search?…")` 대신 `pageService.searchPages()`
 * 를 직접 부른다. (같은 화면의 항목 목록은 그대로 self-fetch 다 — 예외는 검색
 * 결과 한 호출뿐이다.)
 *
 ...
 * 그대로다.
 *
 * **새 예외를 늘리지 마라.** 예외는 여기 하나이며, 추가하려면 같은 수준의
 * 측정 근거(RPC 자체 시간 · 매 요청 재현 · 캐시/정적화 불가 사유)가 필요하다.
 * ─────────────────────────────────────────────────────────────
 *
