...
import { fetchApi, fetchApiAsUser } from "@/lib/api/serverFetch";
...
export default async function AdminPagesPage({
  ...
  // 접고(모르는 status 는 "전체"), 실제로 적용된 값을 응답에 되돌려준다.
  ...
  const [categoryBody, listBody] = await Promise.all([
    ...
    // 세션 쿠키를 실어야 한다. 어드민 목록은 사용자마다 다른 응답이고
    // fetchApiAsUser 가 no-store 로 고정한다 — 캐시되면 초안이 남에게 나간다.
    fetchApiAsUser<AdminPageListBody>(`/api/admin/pages${suffix}`),
  ]);
