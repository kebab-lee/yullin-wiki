...
### 서버 컴포넌트의 self-fetch — 캐시가 왕복을 흡수하지 못하는 화면만 직접 호출

- **기본은 self-fetch 다.** 서버 컴포넌트도 service 를 직접 부르지 않고 자기 Route
  Handler 를 fetch 한다 (`src/lib/api/serverFetch.ts` 의 `fetchApi` /
  `fetchApiAsUser`). 그래야 백엔드가 Java 로 바뀌어도 바꿀 곳이 `getBaseUrl()`
  하나이고, 화면이 보는 계약이 외부 소비자가 보는 계약과 같은 것 하나로 유지된다.
- **갈림길은 하나다: 그 왕복 비용을 캐시가 흡수하는가.** 흡수하면 self-fetch 를
  쓰고, 못 흡수하면 service 를 직접 부른다. **이건 목록이 아니라 판정 기준이다** —
  화면 이름을 외우지 말고 이 질문에 답해라. self-fetch 자체는 공짜가 아니라
  "대개 캐시가 대신 내 주는 비용"이고, 캐시가 없으면 그 비용은 매 요청 그대로 남는다.
  - **흡수한다 → self-fetch.** 공개 읽기 화면이 전부 여기다. 정적 생성이 비용을
    빌드 시점 1회로 접거나(홈 · `/pages/[id]` · `/categories`), `revalidate` 캐시가
    걸린다(`/pages` · `/categories/[slug]`, 그리고 **어느 화면에서든**
    `/api/categories`). → `## 레이아웃` 의 "읽기 페이지는 정적으로 생성된다"
  - **못 흡수한다 → 직접 호출.** 사유는 두 종류뿐이다.
    ① **쿼리마다 URL 이 달라 캐시가 안 걸린다** — `/search`.
    ② **응답이 사용자마다 달라 `no-store` 다** — 세션이 있어야 답이 나오는 화면 전부
    (어드민 · 마이페이지). 둘 다 정적화도 불가능하다(①은 `searchParams`, ②는 세션).
- **현재 직접 호출하는 화면.** 위 기준의 결과일 뿐이므로, 늘거나 줄면 이 줄을 고친다.
  - `/search` — 검색 결과 호출만. 같은 화면의 항목 목록은 캐시가 걸리므로 self-fetch 다.
  - 어드민 7개 — `/admin` · `/admin/pages` · `/admin/comments` · `/admin/reports` ·
    `/admin/users` · `/admin/categories` · `/admin/admins`.
  - 마이페이지 3개 — `/mypage` · `/mypage/edit` · `/mypage/withdraw`.
  - **예외의 예외는 `/admin/pages` 의 `/api/categories` 하나다.** 세션과 무관해
    1시간 캐시가 걸리므로 기준의 ②에 해당하지 않는다. self-fetch 로 남긴다.
- **결과적으로 `fetchApiAsUser` 는 호출자가 없다.** 기준상 그럴 수밖에 없다 —
  그 함수가 필요한 상황(세션 쿠키를 실어야 하는 응답)이 곧 캐시가 안 걸리는
  상황이라 언제나 ②에 걸린다. **그래도 지우지 않는다.** 라우트 핸들러를 남겨 두는
  것과 같은 이유이며(→ 아래 구현 규칙), 위 함수들이 Java 이관 때 되돌아갈 자리가
  그것이다. `fetchApi`(캐시 있는 공개 호출)는 그대로 여러 화면이 쓴다.

#### 직접 호출로 옮길 때의 규칙

- **예외가 사는 자리를 함수 하나로 모은다.** 반환 타입을 라우트 핸들러의 응답
  타입과 **똑같이** 맞춘다(`PageSearchListBody` · `AdminUserListBody` …). 그래야
  JSX 가 한 줄도 바뀌지 않고, Java 이관 시 그 함수 본문만 `fetchApi` /
  `fetchApiAsUser` 로 되돌리면 끝난다. 손댈 파일이 화면 수만큼 늘지 않는 장치다.
- **⚠️ 권한은 페이지가 넘긴다. 이게 이 전환의 가장 큰 위험이다.** 라우트 핸들러가
  `getSession()` 으로 읽어 service 에 넘기던 세션을, 직접 호출에서는 페이지의
  **`requireRole()` 반환값**이 대신 넘긴다. 그래야 service 의 `assertRole` 이
  이전과 똑같이 호출된다. 여기서 `null` 을 넘기거나 인자를 빠뜨리면 **데이터
  가드가 통째로 사라진다** — 화면 가드(`requireRole`)와 데이터 가드(`assertRole`)는
  여전히 두 겹이고 하나가 다른 하나를 대체하지 않는다 (→ `## 권한`).
- **라우트 핸들러를 지우지 마라.** 클라이언트 액션(차단 · 역할 변경 · 상태 전환 ·
  삭제)이 계속 쓰고, 외부 소비자의 계약이자 Java 이관 시 남아야 할 계약도 그쪽이다.
  페이지가 그 라우트를 부르지 않을 뿐 라우트를 없앤 것이 아니다.
- **service 시그니처를 바꾸지 마라.** 라우트와 페이지가 같은 함수를 같은 모양으로
  부르는 상태를 유지한다.
- **쿼리스트링 변환도 함께 넘어온다.** `?page=3` → `3` 은 라우트가 하던 일이라
  직접 호출에서는 페이지가 맡는다. 그 조각은 `src/lib/api/queryParams.ts` 한 벌이며
  이관 시 라우트로 돌아가면서 사라진다. 값의 **범위**를 다듬는 것은 여전히
  service 다(`positiveInt` · `boundedSize` · `parse*Filter`) — 화면이 미리 거르면
  같은 규칙이 두 곳에 생긴다.

#### 측정 근거

- **`/search`** — 검색 RPC 자체는 14.5ms 다(explain analyze, 인덱스 정상). 그런데도
  **첫 요청만이 아니라 매 요청** 느렸으므로 콜드 스타트가 아니라 왕복 비용이다.
  단, **캐시가 0 은 아니다** — 같은 검색어를 연달아 치면 걸린다(로컬 프로덕션
  46ms → 7ms → 7ms). 직접 호출은 그 히트를 포기하는 대신 서로 다른 검색어의 첫
  요청을 29~36ms → 26~28ms 로 낮춘다. 검색은 대부분이 첫 요청이라 이 교환이 이득이다.
- **어드민 · 마이페이지** — TTFB 가 3회 연속 요청에서 모두 비슷하게 느렸고(콜드 스타트 아님),
  응답이 `no-store` 라 fetch 캐시 히트가 애초에 0 이며, 세션 때문에 정적화도
  불가능하다. 로컬 프로덕션 A/B(같은 커밋에서 전/후 두 벌을 빌드해 번갈아 9회씩,
  중앙값·전체 응답 시간):
  `/admin` 45.1→35.2ms · `/admin/pages` 35.4→29.3 · `/admin/comments` 35.0→28.4 ·
  `/admin/reports` 35.5→29.1 · `/admin/users` 34.3→28.2 · `/admin/categories`
  113.3→98.4 · `/admin/admins` 37.6→31.8 · `/mypage` 32.0→27.5 · `/mypage/edit`
  30.6→27.2 · `/mypage/withdraw` 28.8→26.2. 전 화면 2.6~15ms(9~22%) 감소다.
  - **줄어드는 양은 화면이 없앤 왕복 수가 아니라 "직렬 왕복 수"다.** `/mypage` 는
    self-fetch 가 둘이었지만 `Promise.all` 로 나란히 나갔으므로 없어진 것은 한
    번치 대기다. 호출을 더 지운다고 비례해서 빨라지지 않는다.
  - **TTFB 로는 안 보인다.** 스트리밍 SSR 이라 셸이 먼저 나가서 `time_starttransfer`
    는 전후 모두 5~7ms 다. 재려면 **전체 응답 시간**을 봐라.
  - 로컬 수치가 작은 것은 loopback 이기 때문이다. 서버리스에서는 그 왕복이 함수 간
    호출이라 차이가 더 크다.
- **새로 늘릴 때는 같은 수준의 근거를 대라.** ① 쿼리 자체 시간을 재서 병목이 DB 가
  아님을 보이고, ② 매 요청 재현됨(콜드 스타트 아님)을 보이고, ③ 캐시·정적화가 왜
  불가능한지를 댄다. 셋 다 없으면 그 화면은 그냥 self-fetch 다.
