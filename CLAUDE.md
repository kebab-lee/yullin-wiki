...
### 서버 컴포넌트의 self-fetch — 예외는 `/search` 하나뿐이다

- **서버 컴포넌트도 service 를 직접 부르지 않고 자기 Route Handler 를 fetch 한다**
  (`src/lib/api/serverFetch.ts` 의 `fetchApi` / `fetchApiAsUser`). 그래야 백엔드가
  Java 로 바뀌어도 바꿀 곳이 `getBaseUrl()` 하나이고, 화면이 보는 계약이 외부
  소비자가 보는 계약과 같은 것 하나로 유지된다.
- **예외는 `/search` 의 검색 결과 호출 하나다.** `src/app/(site)/search/page.tsx`
  만 `pageService.searchPages()` 를 직접 부른다. 같은 화면의 항목 목록을 포함해
  나머지 self-fetch 는 전부 그대로다.
  - 근거(측정): 검색 RPC 자체는 14.5ms 다(explain analyze, 인덱스 정상). 그런데도
    **첫 요청만이 아니라 매 요청** 느렸으므로 콜드 스타트가 아니라 self-fetch
    왕복이라는 구조적 비용이다. 다른 공개 페이지는 정적 생성으로 그 비용을 빌드
    시점 1회로 흡수했지만(→ `## 레이아웃` 의 "읽기 페이지는 정적으로 생성된다"),
    `/search` 는 쿼리마다 URL 이 달라 fetch 캐시가 걸리지 않고 `searchParams` 를
    읽어 정적화도 불가능하다. 공개 페이지 중 이 비용을 매 요청 내는 화면은
    여기뿐이다.
  - 단, **캐시가 0 은 아니다.** 같은 검색어를 연달아 치면 self-fetch 의 fetch
    캐시가 걸린다(로컬 프로덕션 측정: 46ms → 7ms → 7ms). 직접 호출은 그 히트를
    포기하는 대신 서로 다른 검색어의 첫 요청을 29~36ms → 26~28ms 로 낮춘다.
    검색은 대부분이 첫 요청이라 이 교환이 이득이며, 로컬 수치가 작은 것은
    loopback 이기 때문이다(서버리스에서는 함수 간 호출이라 차이가 더 크다).
  - **`GET /api/pages/search` 는 지우지 않는다.** 클라이언트·외부 소비자의 계약이고
    Java 이관 시 남아야 할 계약도 그쪽이다. 페이지가 그 라우트를 부르지 않을 뿐이다.
  - Java 이관 시 **이 파일 하나는 손대야 한다** — `searchResult()` 본문을 `fetchApi`
    호출로 되돌린다. 응답을 라우트와 같은 `PageSearchListBody` 로 맞춰 두었으므로
    화면 코드는 그때도 무변경이다.
- **새 예외를 추가하지 마라.** 예외는 `/search` 하나뿐이며, 늘리려면 같은 수준의
  측정 근거가 필요하다: ① 쿼리 자체 시간을 재서 병목이 DB 가 아님을 보이고,
  ② 매 요청 재현됨(콜드 스타트 아님)을 보이고, ③ 캐시·정적화가 왜 불가능한지를
  댄다. 셋 다 없으면 그 화면은 예외가 아니라 그냥 self-fetch 다.
